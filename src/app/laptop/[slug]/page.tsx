...
async function getProducts(slug: string): Promise<Product_types | null> {
  return client.fetch(
    groq`*[_type == "laptops" && slug.current == $slug][0]{
      id,
      title,
      slug,
      image,
        ...
        <div className="bg-white shadow-xl rounded-lg overflow-hidden">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 p-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                {laptop.detailImage_2 && (
                  <div className="relative w-full h-48">
                    <Image
                      src={urlFor(laptop.detailImage_2).url()}
                      alt={`${laptop.title} detail 2`}
                      fill
                      className="object-contain rounded-lg"
                    />
                  </div>
                ...
                {laptop.detailImage_3 && (
                  <div className="relative w-full h-48">
                    <Image
                      src={urlFor(laptop.detailImage_3).url()}
                      alt={`${laptop.title} detail 3`}
                      fill
                      className="object-contain rounded-lg"
                    />
                  </div>
            ...
            <div className="space-y-6">
              ...
              <p className="text-xl text-gray-600">Category: {laptop.category}</p>
