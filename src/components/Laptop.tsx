...
import { Heart, ShoppingCart, Star, Eye, Search, Grid3X3, List } from 'lucide-react';
...
const Products = () => {
    ...
    useEffect(() => {
        async function fetch_products() {
            try {
                setLoading(true)
                const products: Product_types[] = await client.fetch("*[_type == 'laptops']")
                    ...
                    <div className={
                        ...
                        {sortedProducts.map((product) => (
                            ...
                            >
                                ...
                                {/* Product Details */}
                               <Link href={`/laptop/${product.slug.current}`}> <div className={`p-4 ${viewMode === 'list' ? 'flex-1' : ''}`}>
                                    ...
                                    {/* Add to Cart Button */}
                                    <button onClick={() => addToCart(product)} className="w-full bg-[#0F172A] hover:bg-[#0F272A] text-white font-semibold py-2 px-4 rounded-lg transition-colors duration-200 flex items-center justify-center gap-2">
